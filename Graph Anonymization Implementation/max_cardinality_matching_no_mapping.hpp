...
#ifndef BOOST_GRAPH_MAXIMUM_CARDINALITY_MATCHING_NO_MAPPING_HPP
#define BOOST_GRAPH_MAXIMUM_CARDINALITY_MATCHING_NO_MAPPING_HPP
...
namespace boost_altered
{
	...
	{
		return boost_altered::is_a_matching(g, mate, get(vertex_index,g));
	}
	...
	public:
		...
		edmonds_augmenting_path_finder(const Graph& arg_g, MateMap& arg_mate, 
			...
			mate(arg_mate),
			ancestor_of_v(n_vertices),
			ancestor_of_w(n_vertices),
			vertex_state(n_vertices),
			origin(n_vertices),
			pred(n_vertices),
			bridge(n_vertices),
			ds_parent(n_vertices),
			ds_rank(n_vertices),

			ds(&ds_rank[0], &ds_parent[0])
		{
		}
		...
		bool augment_matching()
		{
			...
			w_free_ancestor = boost::graph_traits<Graph>::null_vertex();
			v_free_ancestor = boost::graph_traits<Graph>::null_vertex(); 
			...
			while(!even_edges.empty() && !found_alternating_path)
			{
				...
				{                                                             
					...
					while (nearest_common_ancestor == boost::graph_traits<Graph>::null_vertex() && 
						(v_free_ancestor == boost::graph_traits<Graph>::null_vertex() || 
						w_free_ancestor == boost::graph_traits<Graph>::null_vertex()
						)
						)
					{
						...
						if (w_free_ancestor == boost::graph_traits<Graph>::null_vertex())
							w_up = parent(w_up);
						if (v_free_ancestor == boost::graph_traits<Graph>::null_vertex())
							v_up = parent(v_up);

						if (mate[v_up] == boost::graph_traits<Graph>::null_vertex())
							v_free_ancestor = v_up;
						if (mate[w_up] == boost::graph_traits<Graph>::null_vertex())
					...
					if (nearest_common_ancestor == boost::graph_traits<Graph>::null_vertex())
	...
	private:    
		...
		vertex_descriptor_t parent(vertex_descriptor_t x)
		{
			if (vertex_state[x] == graph::detail::V_EVEN 
				&& mate[x] != boost::graph_traits<Graph>::null_vertex())
				return mate[x];
		...
		const Graph& g;
		std::vector<vertex_descriptor_t>& mate;
		VertexIndexMap vm;
		v_size_t n_vertices;

		
		std::vector<e_size_t> ancestor_of_v;
		std::vector<e_size_t> ancestor_of_w;
		std::vector<size_t> vertex_state;
		std::vector<vertex_descriptor_t> origin;
		std::vector<vertex_descriptor_t> pred;
		std::vector<vertex_pair_t> bridge;
		std::vector<vertex_descriptor_t> ds_parent;
		std::vector<v_size_t> ds_rank;

		vertex_list_t aug_path;
		edge_list_t even_edges;
		boost::disjoint_sets< v_size_t*, vertex_descriptor_t* > ds;
	...
	// Input initial matching in mate
	template <typename Graph, typename MateMap>
	...
	struct greedy_matching
	{
		...
		typedef typename boost::graph_traits< Graph >::vertex_iterator vertex_iterator_t;
		typedef typename boost::graph_traits< Graph >::edge_descriptor edge_descriptor_t; 
		typedef typename boost::graph_traits< Graph >::edge_iterator edge_iterator_t;

		static void find_matching(const Graph& g, MateMap& mate)
		{
			//vertex_iterator_t vi, vi_end;
			//for(boost::tie(vi,vi_end) = vertices(g); vi != vi_end; ++vi)
			//	mate[*vi] = boost::graph_traits<Graph>::null_vertex();
	...
	struct extra_greedy_matching
	{
		...
		static void find_matching(const Graph& g, MateMap& mate)
		{
			...
			{
				if( mate[itr->first] == mate[itr->second] ) 
					//only way equality can hold is if mate[itr->first] == mate[itr->second] == null_vertex
				{
					mate[itr->first] = itr->second;
					mate[itr->second] = itr->first;
				}
			}    
		}
	};
	...
	namespace detail
	{
		...
		class odd_components_counter : public boost::dfs_visitor<>
	...
	struct maximum_cardinality_matching_verifier
	{
		...
		static bool verify_matching(const Graph& g, MateMap& mate, VertexIndexMap vm)
		{
			...
			if (!boost_altered::is_a_matching(g,mate,vm))
				return false;
			...
			boost::filtered_graph<Graph, boost::keep_all, non_odd_vertex<vertex_to_int_map_t> > fg(g, boost::keep_all(), filter);
			...
			if (2 * boost_altered::matching_size(g,mate,vm) == num_vertices(g) + num_odd_vertices - num_odd_components)
				return true;
			else
				return false;
		}
	...
	{
		boost_altered::matching < Graph, MateMap, VertexIndexMap,
...
#endif //BOOST_GRAPH_MAXIMUM_CARDINALITY_MATCHING_NO_MAPPING_HPP
